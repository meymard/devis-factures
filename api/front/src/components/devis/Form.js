...
class Form extends Component {
  renderField(data) {
    ...
    if (hasError) {
      data.input['aria-describedby'] = `devis_${data.input.name}_helpBlock`;
      data.input['aria-invalid'] = true;
    }

    return <div className={`form-group${hasError ? ' has-error' : ''}`}>
      <label htmlFor={`devis_${data.input.name}`} className="control-label">{data.input.name}</label>
      <input {...data.input} type={data.type} step={data.step} required={data.required} placeholder={data.placeholder} id={`devis_${data.input.name}`} className="form-control"/>
      {hasError && <span className="help-block" id={`devis_${data.input.name}_helpBlock`}>{data.meta.error}</span>}
    </div>;
  }
...
export default reduxForm({form: 'devis', enableReinitialize: true, keepDirtyOnReinitialize: true})(Form);
